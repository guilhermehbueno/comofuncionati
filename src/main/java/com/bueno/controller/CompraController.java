...
public class CompraController {
	...
	public void twitter() throws Exception {
		// String IKAI_TWITTER_RSS ="http://twitter.com/statuses/user_timeline/14437022.rss";
		...
		paymentRequest.addItem("0001", "Notebook Prata", new Integer(1), new BigDecimal("24.00"), new Long(1000), null);
		paymentRequest.addItem("0002", "Notebook Rosa", new Integer(2), new BigDecimal("25.00"), new Long(750), null);
		...
		if (xml != null && !xml.equals("")) {
			...
			request.setPayload(xml.getBytes(PagSeguroSystem.getPagSeguroEncoding()));
			System.out.println("Payload: "+request.getPayload());
		}

		HTTPResponse response = fetcher.fetch(request);
		System.out.println("Response: " + new String(response.getContent()));
		result.include("contentRetornado",new String(response.getContent()));
	}
